...
def preprocess_sample_orpheus(sample, tokenizer, max_len, g2p, snac_model):
    """
    Preprocess paired sample for Orpheus voice cloning training.
...
def main(config_fpath: str):
    ...
    if use_lora:
        ...
        print(f"LoRA enabled: r={config.get('lora_r', 64)}, alpha={config.get('lora_alpha', 64)}")
    else:
        print("LoRA disabled - using full fine-tuning")
        print("⚠️  Note: Full fine-tuning requires significantly more memory!")
    ...
    wandb.init(
        project=config.get("wandb_project", "Orpheus-Voice-Cloning"),
